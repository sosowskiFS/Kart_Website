...
let PlayerDataUrl = "https://us.kartinvidya.me/data/Playerdata.txt"
let TimeDataUrl = "https://us.kartinvidya.me/data/HardRecords.txt"
/*
let MapDataUrl = ""
let SkinDataUrl = ""
*/
...
function getWinPercent(recordNum) {
	...
	for (const p of cloneList) {
		...
		if (posCounter <= recordNum) {
			...
			pageOutput += (((p[2] / p[1]) * 100).toFixed(2)).toLocaleString("en-US") + "%";
			pageOutput += "</div>"
			pageOutput += "</div>"
			pageOutput += "</li>"
		} else {
...
function getPodiumPercent(recordNum) {
	...
	for (const p of cloneList) {
		...
		if (posCounter <= recordNum) {
			...
			pageOutput += "</div>"
			pageOutput += "<div class='col-3 text-center noPadding'>"
			pageOutput += ( ((p[2] / p[1]) * 100) + ((p[8] / p[1]) * 100) + ((p[9] / p[1]) * 100) ).toFixed(2).toLocaleString("en-US") + "%";
			pageOutput += "</div>"
			pageOutput += "</div>"
			pageOutput += "</li>"
		} else {
...
function getMapRecordCount(recordNum) {
	...
	for (const p of resultArray) {
		if (posCounter <= recordNum) {
			...
			pageOutput += "<div class='col-7 listBorderRight text-center noPadding'>"
			if (p.name.length >= 15) {
				pageOutput += "<span style='font-weight:bold;' class='IAmAnAttentionWhore'>" + p.name + "</span>"
			} else {
				pageOutput += "<span style='font-weight:bold;'>" + p.name + "</span>"
			}
			pageOutput += "</div>"
			pageOutput += "<div class='col-3 text-center noPadding'>"
